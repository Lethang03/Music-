...
const AUDIO_EXTENSIONS = /\.(mp3|wav|m4a|aac|ogg|opus|flac|webm)$/i
function json(body: unknown, status = 200) { return new Response(JSON.stringify(body), { status, headers: { ...cors, 'Content-Type': 'application/json' } }) }
function log(job: { id: string; source_type: string }, event: string, extra: Record<string, unknown> = {}) { console.log(JSON.stringify({ service: 'process-import', job_id: job.id, source_type: job.source_type, event, ...extra })) }
function safeName(value: string) { return value.replace(/[^a-z0-9-_]/gi, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 80) || 'track' }
function titleFromSource(source: string) { const name = source.split('/').pop()?.split('?')[0] || 'Imported Track'; return decodeURIComponent(name).replace(/\.[^.]+$/, '').replace(/[-_]/g, ' ').trim() || 'Imported Track' }
function extensionFor(source: string, contentType: string | null) { const extension = source.match(/\.([a-z0-9]+)(?:$|[?#])/i)?.[1]?.toLowerCase(); if (extension && AUDIO_EXTENSIONS.test(`.${extension}`)) return extension; const map: Record<string, string> = { 'audio/mpeg': 'mp3', 'audio/wav': 'wav', 'audio/x-wav': 'wav', 'audio/mp4': 'm4a', 'audio/aac': 'aac', 'audio/ogg': 'ogg', 'audio/opus': 'opus', 'audio/flac': 'flac', 'audio/webm': 'webm' }; return map[(contentType || '').split(';')[0].toLowerCase()] || 'bin' }
function contentTypeFor(extension: string, received: string | null) { if (received?.toLowerCase().startsWith('audio/')) return received.split(';')[0]; const map: Record<string, string> = { mp3: 'audio/mpeg', wav: 'audio/wav', m4a: 'audio/mp4', aac: 'audio/aac', ogg: 'audio/ogg', opus: 'audio/opus', flac: 'audio/flac', webm: 'audio/webm' }; return map[extension] || 'application/octet-stream' }
...
async function processJob(admin: any, job: any) {
  ...
  try {
    ...
    log(job, 'completed', { track_id: track.id }); return { id: job.id, status: 'completed', track_id: track.id }
  } catch (cause) { const message = cause instanceof Error ? cause.message : String(cause); log(job, 'failed', { error: message }); if (message !== 'Import cancelled.') { const { error } = await admin.from('import_jobs').update({ status: 'failed', progress: 0, error_message: message.slice(0, 1000), completed_at: new Date().toISOString() }).eq('id', job.id).neq('status', 'cancelled'); if (error) console.error('Unable to save import failure', error) }; return { id: job.id, status: message === 'Import cancelled.' ? 'cancelled' : 'failed', error: message } }
}
Deno.serve(async request => {
  if (request.method === 'OPTIONS') return new Response('ok', { headers: cors }); if (request.method !== 'POST') return json({ error: 'Method not allowed.' }, 405)
  const url = Deno.env.get('SUPABASE_URL'); const serviceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY'); if (!url || !serviceKey) return json({ error: 'The import worker is not configured.' }, 500)
  if ((request.headers.get('authorization') || '') !== `Bearer ${serviceKey}`) return json({ error: 'Worker authorization is required.' }, 401)
  const admin = createClient(url, serviceKey, { auth: { persistSession: false } })
  try { const { data: jobs, error } = await admin.rpc('claim_audio_import_job'); if (error) throw error; if (!jobs?.[0]) return json({ processed: false, message: 'No pending import jobs.' }); return json({ processed: true, job: await processJob(admin, jobs[0]) }) } catch (cause) { console.error('process-import invocation failed', cause); return json({ error: cause instanceof Error ? cause.message : 'Unable to process import job.' }, 500) }
})
