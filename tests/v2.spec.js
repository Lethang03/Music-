...
test('manual next and natural ending reuse the persistent global audio element', async ({ page }) => {
  const { errors } = await setup(page, { seconds: 1.2, preferences: { volume: .1, repeat: 'none', autoplay: true } })
  await playFirst(page)
  await page.evaluate(() => { window.firstGlobalAudio = window.testAudio.find(audio => audio.src) })
  await playerBar(page).getByTitle('Next', { exact: true }).click()
  await expect(playerBar(page)).toContainText('Track 2')
  await expect.poll(() => page.evaluate(() => window.testAudio.find(audio => audio.src)?.currentTime || 0)).toBeGreaterThan(0)
  expect(await page.evaluate(() => window.testAudio.filter(audio => audio.src).length)).toBe(1)
  expect(await page.evaluate(() => window.testAudio.find(audio => audio.src) === window.firstGlobalAudio)).toBe(true)
  await expect.poll(() => page.evaluate(() => window.mediaEvents.some(event => event.event === 'ended' && new URL(event.src).pathname === '/test-audio/2.wav')), { timeout: 5000 }).toBe(true)
  await expect(playerBar(page)).toContainText('Track 3')
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('soundverse_playback_diagnostics') || '[]').some(entry => entry.event === 'MANUAL_NEXT'))).toBe(true)
  expect(errors).toEqual([])
})
test('play rejection is surfaced without creating another audio element', async ({ page }) => {
  await setup(page, { rejectPlay: true })
  await page.goto('/music')
  ...
  expect(await page.evaluate(() => window.testAudio.length)).toBe(1)
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('soundverse_playback_diagnostics') || '[]').some(entry => entry.event === 'PLAY_REJECTED'))).toBe(true)
})
